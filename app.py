...
@app.route('/r/<int:user_id>')
def get_clone_bot_username(user_id):
    """
    Called by Cloudflare Worker when it detects a clone bot link.
    Returns current active clone bot username for that user_id.
    Worker redirects to t.me/CloneBotUsername?start=ENCODED
    """
    import asyncio
    from handlers.database import db as database

    async def _get():
        clone = await database.get_clone(user_id)
        if clone and clone.get('bot_username'):
            return clone['bot_username']
        return None

    try:
        loop = asyncio.new_event_loop()
        username = loop.run_until_complete(_get())
        loop.close()
        if username:
            return jsonify({"username": username})
        return jsonify({"error": "Clone bot not found"}), 404

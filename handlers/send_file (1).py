...
def _get_stream_buttons(file_id: int, filename: str = "file"):
    ...
    try:
        ...
        stream_link = get_stream_link(file_id, filename)
        dl_link = get_download_link(file_id, filename)
        return [[
...
async def media_forward(bot: Client, user_id: int, file_id: int):
    ...
    try:
        # Get message to extract filename for stream links
        original_msg = await bot.get_messages(
            chat_id=Config.DB_CHANNEL,
            message_ids=file_id
        )
        filename, _ = get_file_info(original_msg) if original_msg else ("file", 0)
        if filename == "Unknown":
            filename = "file"

        buttons = _get_stream_buttons(file_id, filename)

        ...
        async def clone_save(bot, message):
            try:
                ...
                if Config.WORKER_URL:
                    ...
                    serve_username = Config.SERVE_BOT_USERNAME or me.username
...
async def clone_handler(bot: Client, m: Message):
    ...
    if success:
        ...
        # Auto-promote clone bot as admin in DB_CHANNEL
        try:
            from pyrogram.types import ChatPrivileges
            clone_instance = clone_bots.get(m.from_user.id)
            if clone_instance:
                clone_me = await clone_instance.get_me()
                await bot.promote_chat_member(
                    chat_id=Config.DB_CHANNEL,
                    user_id=f"@{clone_me.username}",
                    privileges=ChatPrivileges(
                        can_post_messages=True,
                        can_delete_messages=True
                    )
                )
                logging.info(f"Clone bot @{result} auto-promoted in DB_CHANNEL ✅")
        except Exception as e:
            logging.warning(f"Could not auto-promote clone bot @{result} in DB_CHANNEL: {e}")

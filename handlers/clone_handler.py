        ...
        async def clone_start(bot, cmd):
            ...
            if start_param:
                ...
                try:
                    ...
                    # Get custom caption if set
                    custom_caption = settings.get('custom_caption')
                    me = await bot.get_me()
                    share_link = f"https://t.me/{me.username}?start=mrkiller_{str_to_b64(str(file_id))}"

                    # Build caption
                    if custom_caption:
                        media = get_msg.document or get_msg.video or get_msg.audio
                        ...
                        try:
                            ...
                            caption_text = custom_caption.format(
                                ...
                                caption=getattr(get_msg, 'caption', '') or ''
                            )
                        except Exception:

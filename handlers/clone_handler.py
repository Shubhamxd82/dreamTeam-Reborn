...
# Main bot client — used to save files to DB_CHANNEL on behalf of clone bots
# Clone bots can't access DB_CHANNEL directly since they're not members
main_bot_client = None

def set_main_bot(client):
    """Called from bot.py at startup to pass the main bot client."""
    global main_bot_client
    main_bot_client = client
    ...
    try:
        ...
        async def clone_save(bot, message):
            try:
                ...
                # Extract media and file_id from the message
                ...
                # Use main bot to send file to DB_CHANNEL using file_id
                # This works because file_ids are global across bots on same API
                if main_bot_client:
                    if media_type == "video":
                        forwarded = await main_bot_client.send_video(
                            chat_id=Config.DB_CHANNEL,
                            video=tg_file_id,
                            caption=getattr(message, 'caption', '') or ''
                        )
                    elif media_type == "audio":
                        forwarded = await main_bot_client.send_audio(
                            chat_id=Config.DB_CHANNEL,
                            audio=tg_file_id,
                            caption=getattr(message, 'caption', '') or ''
                        )
                    else:
                        forwarded = await main_bot_client.send_document(
                            chat_id=Config.DB_CHANNEL,
                            document=tg_file_id,
                            caption=getattr(message, 'caption', '') or ''
                        )
                ...
                # Use WORKER_URL like save_media.py — all links via worker.dev
                encoded = str_to_b64(file_id_str)
                if Config.WORKER_URL:
                    share_link = f"{Config.WORKER_URL}/mrkiller_{encoded}"
                else:
                    serve_username = Config.SERVE_BOT_USERNAME or Config.BOT_USERNAME
                    share_link = f"https://t.me/{serve_username}?start=mrkiller_{encoded}"
        ...
        # Auto-promote clone bot in DB_CHANNEL so it can forward files there
        # This is called from clone_handler which has the main bot client
        # We store the me.id for the caller to use

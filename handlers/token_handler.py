...
from handlers.helpers import get_token_shortlink, format_time_seconds, str_to_b64
...
async def check_token(user_id: int) -> bool:
    ...
    if not token_data or not token_data.get("is_verified"):
        return False
...
async def generate_token_link(bot_username: str, user_id: int) -> str:
...
async def get_token_msg(user_id: int, bot_username: str) -> tuple:
    """Get the verification message and button for a user."""

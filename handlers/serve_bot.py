...
async def save_to_db_channel(file_id: str, media_type: str, caption: str = ""):
    """
    Save a file to DB_CHANNEL using the serve bot session.
    Called from clone_handler — serve bot IS admin in DB_CHANNEL.
    """
    if not serve_bot_instance:
        raise Exception("ServeBot not initialized")
    if media_type == "video":
        return await serve_bot_instance.send_video(
            chat_id=Config.DB_CHANNEL,
            video=file_id,
            caption=caption
        )
    elif media_type == "audio":
        return await serve_bot_instance.send_audio(
            chat_id=Config.DB_CHANNEL,
            audio=file_id,
            caption=caption
        )
    else:
        return await serve_bot_instance.send_document(
            chat_id=Config.DB_CHANNEL,
            document=file_id,
            caption=caption
        )

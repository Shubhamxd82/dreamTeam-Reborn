...
async def start_serve_bot(main_bot: Client):
    ...
    # Auto-promote serve bot as admin of DB_CHANNEL
    # so it can read and copy files from there
    try:
        await main_bot.promote_chat_member(
            chat_id=Config.DB_CHANNEL,
            user_id=me.id,
        ...
        logging.info(f"FileServeBot @{me.username} auto-promoted in DB_CHANNEL ✅")
    except Exception as e:
        logging.warning(
            f"Could not auto-promote ServeBot in DB_CHANNEL: {e}\n"
            "Please manually add @{me.username} as admin to your DB_CHANNEL."
        )

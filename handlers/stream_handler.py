...
# ── Middleware (bypasses aiohttp path normalization for + in filenames) ──────────

@web.middleware
async def stream_middleware(request, handler):
    raw_path = request.raw_path.split("?")[0]

    if raw_path.startswith("/watch/"):
        rest = raw_path[len("/watch/"):]
        try:
        ...
        try:
            base_url = Config.get_stream_base_url()
            filename = rest.split("/")[1] if "/" in rest else "file"
            h = _make_hash(msg_id)
    ...
    elif raw_path.startswith("/stream/"):
        rest = raw_path[len("/stream/"):]
        try:
            msg_id = int(rest.split("/")[0])
        except ValueError:
            return await handler(request)
    ...
    elif raw_path.startswith("/dl/"):
        rest = raw_path[len("/dl/"):]
        try:
            msg_id = int(rest.split("/")[0])
        except ValueError:
            return await handler(request)
